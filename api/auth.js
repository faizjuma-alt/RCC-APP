 ...
 *
 * Actions:
 *   POST ?action=request  — whitelist check + send magic link email via Gmail API (OAuth2)
 ...
 *   GMAIL_USER           — Gmail address to send from (e.g. faiz.juma@jumia.com)
...
async function findUserInSheet(email) {
  ...
  for (let i = 0; i < Math.min(allRows.length, 6); i++) {
    ...
    if (eIdx >= 0 && nIdx >= 0) { headerIdx = i; emailCol = eIdx; nameCol = nIdx; break; }
  }

  if (headerIdx >= 0) {
    const userRow = allRows.slice(headerIdx + 1).find(row =>
      String(row[emailCol] || '').toLowerCase().trim() === cleanEmail
    );
    if (userRow) return { email: cleanEmail, name: String(userRow[nameCol] || '').trim() || formatName(email) };
  }
  ...
  try {
    ...
    const mgrRow = (mgrRes.data.values || []).find(row =>
      row[0] && String(row[0]).toLowerCase().trim() === cleanEmail
    );
    if (mgrRow) return { email: cleanEmail, name: mgrRow[1] ? String(mgrRow[1]).trim() : formatName(email) };
  } catch (e) {
    console.warn('Managers tab not found:', e.message);
  }
          ...
          <tr>
            <td style="padding:36px 32px;">
              ...
              <p style="margin:28px 0 0;font-size:13px;color:#9CA3AF;line-height:1.6;">
                ...
                Link not working? <a href="${magicUrl}" style="color:#F68B1E;word-break:break-all;">${magicUrl}</a>
              </p>
            </td>
          </tr>
          <tr>
...
module.exports = async function handler(req, res) {
  ...
  if (action === 'request') {
    ...
    let user;
    try { user = await findUserInSheet(email); }
    catch (err) {
    ...
    if (!user) return res.status(403).json({ success: false, error: 'Email not registered. Contact your manager to be added.' });
    ...
    if (!process.env.GMAIL_USER || !process.env.GMAIL_CLIENT_ID || !process.env.GMAIL_REFRESH_TOKEN) {
      console.log('--- MAGIC LINK (no Gmail OAuth2 credentials set) ---');
  ...
  if (action === 'verify') {
    ...
    try { payload = verifyToken(token); }
      ...
      const msg = err.name === 'TokenExpiredError' ? 'Link has expired. Please request a new one.' : 'Invalid link. Please request a new one.';
      return res.status(401).json({ success: false, error: msg });
    }
    if (payload.type !== 'magic') return res.status(401).json({ success: false, error: 'Invalid token type' });
    const sessionToken = signToken({ email: payload.email, name: payload.name, type: 'session' }, '30d');
    return res.status(200).json({ success: true, sessionToken, user: { email: payload.email, name: payload.name } });
  }
  ...
  if (action === 'me') {
    ...
    if (payload.type !== 'session') return res.status(401).json({ success: false, error: 'Invalid token type' });
    return res.status(200).json({ success: true, user: { email: payload.email, name: payload.name } });
  }
